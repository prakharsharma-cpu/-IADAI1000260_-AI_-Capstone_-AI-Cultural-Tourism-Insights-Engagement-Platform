...
# Fully Integrated, Gemini-powered, Multi-feature
# ===============================================
...
# Session state for gamification & datasets
for key, default in {
    "points": 0, "level": 1, "achievements": [],
    "datasets": {}, "selected_dataset": None, "master_name": None,
    "messages": []
}.items():
...
st.sidebar.markdown("""
- 🎙️ Voice (upload placeholder)
- 📱 Social sharing
- 🎮 Gamification (points & badges)
- 📈 Live Trends (from CSVs)
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 📂 Upload reference CSVs (multiple)")
    ...
    for f in uploaded_files:
        try:
            ...
            try: df = pd.read_csv(f, encoding='latin1')
                ...
                st.sidebar.warning(f"Could not read {f.name}.")
                continue
...
if page == "🔎 EDA (Plotly) & Data Prep":
    st.header("🔎 Interactive EDA with Plotly")
    if not st.session_state.datasets:
        st.info("Upload CSVs from sidebar to start EDA.")
        st.stop()

    dataset_names = list(st.session_state.datasets.keys())
    ds_choice = st.selectbox("Select dataset", dataset_names,
                             index=dataset_names.index(st.session_state.selected_dataset)
                             if st.session_state.selected_dataset in dataset_names else 0)
    ...
    with st.expander("Dataset summary"):
        meta = pd.DataFrame({"column": df.columns,
    ...
    if plot_type == "Histogram":
        ...
        else: st.warning("No numeric columns for histogram.")
        ...
        if categorical_cols:
            ...
            st.plotly_chart(fig, use_container_width=True)
        else: st.warning("No categorical columns available.")
        ...
        if len(numeric_cols) >= 2:
            ...
            fig = px.scatter(df, x=xcol, y=ycol, color=color, title=f"{ycol} vs {xcol}", hover_data=df.columns)
            st.plotly_chart(fig, use_container_width=True)
        else: st.warning("Need at least two numeric columns.")
    elif plot_type == "Correlation heatmap":
        if numeric_cols:
            corr = df[numeric_cols].corr()
            fig = px.imshow(corr, text_auto=True, aspect="auto", title="Correlation matrix")
            st.plotly_chart(fig, use_container_width=True)
        else: st.warning("No numeric columns for correlation.")

    if st.button("🔖 Use this dataset for the app"):
        st.session_state.master_name = ds_choice
        st.success(f"{ds_choice} set as master dataset.")
        award_points(50)
...
# =========================
    ...
    st.header("✈️ Premium AI Itinerary (Gemini)")
    master_df = st.session_state.datasets.get(st.session_state.master_name, None)
    if master_df is None: master_df = next(iter(st.session_state.datasets.values()))
    ...
    if submit:
        dest_cols = [c for c in master_df.columns if "dest" in c.lower() or "name" in c.lower() or "site" in c.lower()]
        dest_list = master_df[dest_cols[0]].dropna().unique().tolist()[:30] if dest_cols else []
        prompt = (f"Create a professional {days}-day itinerary in {language} for interests {', '.join(interests)}.\n"
                  f"Budget: {budget}.\nDestinations: {dest_list}\nInclude schedule, transport, culture, tips, emojis.")
...
# =========================
    ...
    st.header("⭐ Smart Recommendations (AI + data-driven)")
    master_df = st.session_state.datasets.get(st.session_state.master_name, None)
    if master_df is None:
        st.info("Set a master dataset for recommendations."); st.stop()
    ...
    interest = st.selectbox("Interest", ['Culture','History','Nature','Food','Architecture'])
    season_opt = st.selectbox("Season", sorted(master_df['Best Season'].dropna().unique()) if 'Best Season' in master_df.columns else ["Any"])
    ...
    if type_cols:
        df_candidates = df_candidates[df_candidates[type_cols[0]].str.contains(interest, case=False, na=False)] if df_candidates[type_cols[0]].dtype==object else df_candidates
    if rating_cols:
        df_candidates = df_candidates.sort_values(by=rating_cols[0], ascending=False)
    recs = df_candidates.head(8)

    st.markdown(f"### Top recommendations ({len(recs)})")
    for i, row in recs.iterrows():
        cols = st.columns([1,4,1])
        rating_display = f"{row[rating_cols[0]]:.1f}" if rating_cols else "-"
        with cols[0]: st.metric("⭐", rating_display)
        with cols[1]:
            title = next((row[c] for c in row.index if "name" in c.lower() or "destination" in c.lower() or "site" in c.lower()), row.index[0])
            st.markdown(f"**{title}**")
            info = [f"{c}: {row[c]}" for c in ['Country','Type','Best Season'] if c in row.index]
            st.caption(" | ".join(info))
        with cols[2]:
            if st.button("❤️ Like", key=f"like_{i}"): award_points(25)
...
# =========================
    ...
    fb, tw, ig = st.columns(3)
    if fb.button("Share to Facebook"):
        st.success("Facebook link generated.")
        st.code(f"https://facebook.com/sharer.php?u=https://your-app-url.example&quote={story}")
        award_points(75)
    if tw.button("Share to Twitter/X"):
        st.success("Twitter link generated.")
        st.code(f"https://twitter.com/intent/tweet?text={story[:250]}")
        award_points(75)
    if ig.button("Instagram Story Tip"):
        st.info("Use caption with photo for IG Story.")
        award_points(50)
...
# =========================
    ...
    itinerary_text = st.text_area("Paste itinerary or report", "Day 1: ...\nDay 2: ...")
...
# =========================
    ...
    st.header("🎬 Trip Recap Video")
    uploaded_imgs = st.file_uploader("Upload images (max 10)", accept_multiple_files=True, type=["png","jpg","jpeg"])
    duration_per_slide = st.slider("Seconds per slide", 1, 6, 2)
    if uploaded_imgs and st.button("Generate Video"):
        try:
            from moviepy.editor import ImageClip, concatenate_videoclips
        except Exception:
            st.error("Install moviepy to enable video generation."); st.stop()
        clips = [ImageClip(np.asarray(Image.open(f).convert("RGB"))).set_duration(duration_per_slide) for f in uploaded_imgs[:10]]
        final = concatenate_videoclips(clips, method="compose")
        tmpfile = "recap.mp4"
        final.write_videofile(tmpfile, fps=24, codec="libx264", audio=False, verbose=False, logger=None)
        st.video(open(tmpfile,"rb").read())
        award_points(150)
...
# =========================
    ...
    st.header("💬 Multilingual Smart Chat (Gemini)")
    for msg in st.session_state.messages:
        with st.chat_message(msg['role']): st.markdown(msg['content'])
    if prompt := st.chat_input("Ask about destinations, itineraries, datasets..."):
...
# =========================
    ...
    st.header("📊 Analytics & Live Trends")
    if not st.session_state.datasets: st.info("Upload datasets to compute trends."); st.stop()
    combined = pd.concat([df.assign(__source=name) for name, df in st.session_state.datasets.items()],
    ...
    if name_cols and metric_cols:
        name_col, metric_col = name_cols[0], metric_cols[0]
    ...
    st.markdown("---")
    st.markdown("### Live Feature Metrics (simulated)")
    ...
    if st.button("✨ Claim analytics reward (+100 pts)"): award_points(100); st.success("Points added.")

# -------------------------
# Footer
# -------------------------
st.markdown("---")
st.caption("Built with Gemini GenAI • Plotly • Streamlit • Fully personalized
